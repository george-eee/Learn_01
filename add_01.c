...
//1_移植性原因：不是所有硬件平台都能访问任意地址的数据 有些平台只能访问特定地址的数据 所以为了提高程序的移植性 采用内存对齐的方式
//2_性能原因： 数据结构(尤其是栈)应该尽可能的在自然边界上进行对齐 这是因为处理器访问未对齐的数据时需要进行两次内存访问操作
// 而对齐的数据访问则只需要进行一次内存访问操作
//解释： 如果让X86处理器访问以下结构体的数据：
//struct stu
//{
//	char c;
//	long a;
//};
//由于存在内存对齐 所以对于X86处理器而言只需要访问两次(一次8Byte) 同时每次得到的数据都是完整的
//如果没有内存对齐 那么该处理器访问时 c的1字节 和 a的8字节联合在一起 第一次访问时访问到了c和a的前面7个字节
//这时需要第二次访问才能补齐a的完整数据 这样虽然也只需要访问两次 但是每次得到的数据不一定是完整的
//这反而会加大访问难度
//总结来说--结构体的内存对齐是"以空间换时间的做法"
//那么 如何让我们浪费的空间尽可能的少呢？
//答案是---->>让占用内存空间小的成员变量尽可能的靠在一起 例如上面示例代码中的s2 而不采用s1

//这里VS中的默认对齐数其实是可以修改的 需要通过预处理指令：#pragma
//例如：#pragma pack(4) 就是将默认对齐数修改为4！#pragma pack() 则是恢复默认对齐数
//甚至我们还可以将其修改为无对齐数--->#pragma pack(1) 以1为对齐数其实就是无对齐数 每个数据之间都是相连的
//不过还是不要随意使用最好 :)

//结构体传参：传值/传址
//print(struct s s1)或print(const struct s* ps1) 
//优先推荐传址的方式！ 因为传值会浪费很大的栈区空间 同时也会导致压栈操作过久 浪费很多时间
//这里加上const修饰是为了避免对原结构体变量产生影响

//位段(又称位域)：位是bit位的意思 用于表示结构体中某些成员变量的bit位数的限制 使用形如：
//struct bitfield
//{
	//int a : 2;//冒号后面的是bit位宽度
	//int b : 15;
	//int c : 20;
	//int d : 12;//最初C语言规定时只规定了int类型可以使用 但是事实上 整型家族的成员都可以使用位段的特性
//};//之和是49bit 即至少要7Byte
//int main()
//{
//	printf("%zu\n", sizeof(struct bitfield));//8  为什么不是7 而是8？ 因为位段会在分配内存空间时允许有一定程度的浪费
	//例如此处int起始 所以分配4个字节 也就是32bit 然后再分配给成员变量c时不够用了 这32bit位剩余的15就被浪费掉
	//然后由于接下来的是int类型 所以重新开辟32个bit位 刚好装下c和d 所以最后的结果是8
	//这也就是一个"知足常乐"的道理 因为原先这里要用到16Byte 现在少了8Byte的浪费 已经是节省了空间了
//}
//以上我们只是以一个全int类型位段来示例的 事实上 使用位段同样要注意结构体内存对齐！
//struct bit
//{
//	char a : 2;
//	int b : 3;
...
//int main()
//{
//	printf("%zu\n", sizeof(struct bit));//16
//	//这是因为a开辟一个字节后限制宽度为2bit 然后到b时要内存对齐 所以要先额外浪费3个字节 然后b开辟4个字节
//	//到c时也要内存对齐 但此时偏移量刚好是8 所以不必再浪费额外空间了 直接开辟c的8个字节(64bit) 显然明显够存下c和d 所以结果是16
//	return 0;
//}
//位段的具体的内存分配方式？
//在VS中 以一个字节为例 位段中bit位的分配是从低到高的 例如char a:2 ; a会占据字节中的最低两位 char b:3 ; 则b会向更高位占据三bit
//注意 这会导致：当我们给位段结构体变量中的成员变量进行赋值时 最终在内存中存的并不是我们开始想要的值 例如：
//char a:2 ; char b: 3;  赋值：s.a=0 s.b=10 10的二进制是1010 按照位段的内存分配方式 最终会得到10100000--->>160
//而不是我们以为的10！

//位段其实也是一种具有风险的特性 因为它的不确定性同样很高 主要体现在：
//位段的跨平台问题：
//1_int位段被当成有符号还是无符号并不确定
//2_位段中最大位的数目不能确定 16位机器中int类型占2个字节 16bit 而32位机器中int类型占4字节 32bit 那么从32位机器上移植代码时
// 就可能会出现溢出的问题 如int a: 30; 这段代码在16位机器上就会出现问题
//3_位段中的成员在内存中单个字节里的分配顺序不确定 可能是从低位向高位 也可能是高位向低位
//4_当一个位段成员剩余的bit位不足以容纳下一个位段成员时 这部分剩余的bit位是浪费还是利用 也是不确定的
//总结：位段能在不影响结构体功能的同时尽可能地节省空间 但是会有不兼容跨平台的问题！

//位段的应用：以计算机网络中的应用为例...
//计算机网络中 当我们需要传输一个数据时 会对数据进行封装加码 包括源ip地址/目的ip地址/标识符/长度/偏移等等
//这些封装确保了我们数据的传输无误 而他们占的大小也各不相同 有4bit 8bit 13bit 16bit 32bit等等
//如果我们统一用int类型的大小 会导致传输数据过大 传输损耗变大 所以最好的方式是按照各个封装不同的大小分配不同的空间
//这时就需要用到位段了 事实上 至今计算机网络的底层实现中都包含了位段这一特性


//通讯录...


//枚举：生活中有很多量是可以一一列举出来的 这时我们就可以用到枚举
//枚举类型的定义：
//enum day//定义一个day枚举类型----->>>>联想结构体 此处只是声明了一种类型
//{
//	Mon,//用, 而不是;
//	Tues,
//	Wed,
...
//int main()
//{
//	//enum day d5 = Fri;//d5是一个枚举变量 它的取值只能在该枚举类型中的枚举常量中取
//	//枚举常量也是有值的！
//	printf("%d\n", Mon);//0 默认枚举常量的值从0开始--->>类似于数组下标 只不过这个值是可以修改的 在声明枚举类型时：
//	//Mon=1;   这样就将第一个枚举常量的值修改成了1 并且 后续的Tues也会变成2 Wed变成3...
//	printf("%d\n", Tues);//1
//	printf("%d\n", Wed);//2
//	printf("%d\n", Thur);//3
//	printf("%d\n", Fri);//4
...
//	printf("%d\n", Sun);//6
//	return 0;
//}//枚举和穷举的区别：前者是明确举出的总例有限 为了方便采用枚举列出 而后者是尽可能地列举出所有可能 往往总例很多并且当时不明确

//枚举的优点：
//1_增加代码的可读性与可维护性
// 当我们在写通讯录时 假定的用户输入1 则是增加 2则是减少..然后后面同样用case 1 2..来对应跳转到相应的函数
// 但是这样并不方便 我们写case 1 2时还要返回去看一下对应的选项 如果我们用枚举类型 装下exit add... 后面就可以直接写case exit...
// 显然要更加方便快捷 并且易于后续维护
//2_和#define定义的标识符相比有类型检查 更加严谨
// 这一点在C语言上体现不明显 因为C语言的编译环境不那么严格 但是如果切换成cpp 会发现enum day d5=5是跑不过去的
// 因为默认d5是枚举类型 而5是整型 这样的类型检查会让我们在运行前就及时发现错误
//3_防止了命名污染(封装)
// 将常量名集中放置 避免后续重复命名
//4_便于调试
// 相比于#define 枚举类型中的枚举常量不会被替换掉(替换成对应的值) 而#define定义的常量会在编译前(预处理时)被替换成值 这样的话就不利于调试
//5_使用方便 一次性可定义多个常量


//联合(我们联合！)：也叫共用体 联合也是一种自定义类型 联合类型定义的变量往往包含一系列成员 这些成员共用一块内存空间(共用体)
//枚举类型的定义：
//union un
//{
//	int a;
...
//{
//	union un u;
//	printf("%zu\n", sizeof(union un));//4? 按理来说不应该最少都是5吗 这其实就是联合体的特征！所有成员共用一片内存空间
//	//所以自然是以最大的那一个成员的内存空间为共用的内存空间了
//	printf("%p\n", &u);//00000067356FF664
//	printf("%p\n", &u.a);//00000067356FF664
//	printf("%p\n", &u.c);//00000067356FF664
//	//意外吗？这个联合类型变量和它的成员的地址全都是一样的 这也再次印证了：所有成员共用一片内存空间的特征
//	return 0;
//}
//由于共用空间的特征 如果同时使用联合体中的多个成员 很可能会导致其中小内存空间的成员被大内存空间的成员给覆盖掉 
//因为所有成员都是从首字节空间开始存放数据的
//所以联合体的典型应用场景其实是每次只使用其中一个成员 其他成员不使用
//用A不用B C D.. 用B不用A C D.. 这种联合体的形式可以让我们在一片空间中自由地调用想调用的成员/类型
...
//{
//	union un u;
//	u.a = 0x44332211;//按照小端存储 第一个字节应该是0x11
//	u.c = 0x00;//将u.c赋值为0x00
//	printf("%x\n", *(char*)&u);//只打印第一个字节 00
//	//这说明联合体中成员共用同一片空间 并且起始地址都是相同的
//	//这也意味着 如果同时操作联合体的成员 必然会意味着覆盖操作 这也是为什么联合体的使用场景一般都是调用单独的成员
//	return 0;
//}
...
//		int i;
//	}u;
//	u.i = 1;//将u.i赋值为0x00000001 即数据低位赋值为1 
//	return u.c;//u.c占用的空间显然是联合体中的第一个字节 也就是低地址 通过返回u.c的值 可以判断低地址中到底存放的什么数据
//}
//int main()
//{
//	int ret=check_sys();
...
//	char c[10];//10
//	int i;//4
//};//按直觉而言 这里占用的内存应该是更大字节的 也就是10
//int main()
//{
//	printf("%zu\n", sizeof(union un));//但是结果是12 因为char对齐数是1 int对齐数是4 所以最大对齐数是4 最终大小必然是4的倍数
//	//10往后申请到4的倍数 也就是12 但是联合体仍然是共用10字节 后面的2字节是直接浪费掉了的
//	return 0;
//}
//总结：
//1_联合体的大小至少!是最大成员的大小;
//2_当联合体的大小不满足最大对齐数的整数倍时则对齐到最大对齐数的整数倍(对齐时申请的额外空间是不使用的 直接浪费掉);


//动态规划----->>>>
//为什么需要动态规划？
//像我们如果用int arr[10] 来创建一个有10个变量的数组 那么这个数组的长度就是固定的
//虽然我们说有变长数组这个东西 但是请注意! 变长数组只是说数组的长度在创建前可以是未定的  创建后也必然是固定的
//但是很多时候 这些变量/数组的大小我们只有在程序运行起来之后才能知道 这种情况下我们就只能试试动态内存开辟了
//动态规划就4个核心的动态内存函数：均要引用头文件--->>stdlib.h
//malloc calloc realloc free

//1_malloc:
//范式： void* malloc(size_t size) size表示申请的内存字节数 void*能隐式类型转换为任意类型
//这个函数向内存中申请一块连续可用的空间 并返回指向这片空间的指针
//如果开辟成功 则返回指向该空间的指针; 如果开辟失败 则返回一个空指针(所以malloc的返回值一定要做检查)
//此外 如果size=0 这是C语言标准未定义的 取决于编译器
//使用示例：
//int main()
//{
...
//	if (p==NULL)//如果开辟失败 则-->>
//	{
//		printf("%s\n", strerror(errno));//这里打印失败原因(勿忘引用头文件string.h和errno.h)
//		return 1;//即使终止程序 以免后续继续执行 <习惯上我们用返回0代表执行正常 返回1代表执行出错>
//		//这里为了演示效果 我们用INT_MAX来试验一下  INT_MAX是C语言内置的最大(有符号)整形 也就是2^31-1:2147483647
//		//但是这里我们用X64会正常运行 为了观察 我们切换到X86 会发现报警告：Not enough space 表明开辟空间失败
//	}
//	for (int i = 0; i < 10; i++)
//	{
//		*(p + i) = i;//这里的赋值可以在监视窗口中查看 只需要输入p,10即可弹出类似于数组的观察视角
//	}
//	for (int i = 0; i < 10; i++)
//	{
//		printf("%d ", *(p + i));
//	}
//	//这里我们没用到free 按理来说 如果不用free的话 不是会造成内存泄漏吗？
//	//但是 事实上即使我们不用free 系统也会自动在程序退出时回收内存空间 
//	//这里仍然有个问题但是！如果我们这个程序需要二十四小时不停运行 那么这就必然会造成内存泄漏 
//	//所谓内存泄露就是内存被内存函数要走了 但是不归还 这就会导致内存自己的可支配空间越来越少 而那些"走失"的内存就被称为泄露的内存
//	//所以不用free仍然是有极大安全隐患的 所以我们下面添加上free：
//	free(p);//free函数 范式void free(void* ptr) ptr指向欲释放掉的内存空间 free函数可以用来释放程序员申请走的空间 使其回归系统
//	p = NULL;//但是 free函数不能清空掉p指针中存放的地址 如果这里不对p赋值NULL 则会导致p变成一个野指针 更加危险！
//	return 0;
//}
...
//	while (1)
//	{
//		void* p=malloc(100);//这里有意让程序陷入死循环 最终通过任务管理器我们会发现VS调用了约1.9G的内存 便已经是极限了
//		//事实上 这个值是可以计算出来的 涉及到计组的知识
//	}
//	//这样的代码其实相当危险！在较早期的OS版本中 这样不断地内存泄漏可能会直接导致OS崩溃 
//  //后来随着OS设计的完善 限制了内存分配 才避免了直接性的系统崩溃 但是这种无节制的开辟内存空间对于OS而言仍然相当危险！
//	return 0;
//}

//2_free
//范式： void free(void* ptr) ptr指向欲释放/回收的内存空间(的起始空间)
//free函数就是用来释放/回收动态开辟的内存的
//A_如果ptr指向的空间不是动态开辟的内存空间 那么此时free函数的行为是未定义的
//B_如果ptr是空指针 则函数不做任何行为

//3_calloc
//范式： void* calloc(size_t num ,size_t size)
//与malloc的区别：A_calloc是用于开辟num个大小为size的内存空间 而malloc是直接开辟size个内存空间 (这些空间都在堆区上)
//B_calloc会在返回地址之前将空间中的每个字节都初始化为0 而malloc不会
//总结 如果我们确定要开辟的元素类型/我们想要让其在开辟好时就初始化   则用calloc
//反之 如果我们不确定要开辟的元素类型/我们不想让其直接初始化为0		 则用malloc

//4_realloc
//范式： void* realloc(void* ptr ,size_t size) 如果ptr是NULL 则此时realloc等同于malloc！
//ptr是指向要修改的内存空间的起始空间的指针 size是修改后内存空间的大小 
//从realloc的名字就可以看出来 它可以说是动态开辟函数的灵魂 因为它大大提高了动态规划的灵活性
//它的使用场景是 有时我们会觉得开辟的空间大了 有时又觉得小了 而realloc就可以实现及时地修改开辟的空间
//同时 realloc函数在调整原空间的大小的基础上还会将原空间的数据迁移到新的空间中
//这里就不得不提到realloc在调整内存大小的两种情况了(一般是增大)：
//A_原空间之后有足够大的空间
// 那么realloc函数会直接在原空间后追加新增的的空间 不改动原开辟空间的地址
//B_原空间之后没有足够大的空间
// 此时realloc函数会自动在内存中向后进行寻找 直到寻找到足够放得下修改后内存大小的空间 然后将原空间数据迁移过来
// 同时再将原空间释放掉 使用新开辟的空间 此时realloc返回的地址自然与原空间不相同
//所以如果我们在realloc之后仍然想要使用原先使用的指针p 可以先用一个新指针接收返回地址 然后将新指针的地址赋给原指针p
//但是这样做也还是有风险 因为可能realloc开辟失败了 返回了一个空指针 这时就会将p也初始化为NULL 导致后续在操作p时出现程序崩溃的可能
//所以在赋(新)地址给p之前 要先对realloc返回的(新)地址进行检查 判断是否是NULL 然后再赋值

//事实上 从realloc调整内存空间的两种可能就可以看出 系统在堆区上给这些动态内存开辟函数开辟空间时 并不是连续开辟的！
//上一个和下一个开辟空间之间是有间隔的 而且这个间隔也并不固定
//所以如果我们频繁使用动态内存开辟函数 必然会导致堆区空间变得零零散散 相互错落 而这些碎片空间如果不能利用起来----->>>
//(事实上本身也很难利用起来) 又势必会造成内存空间的极大浪费 此外 频繁使用这些函数还会导致程序效率下降(因为系统开辟空间也需要时间)
//PS--->>这里有一个相关的项目叫"高并发内存池":就是让系统在一开始就分配够用且稍大的空间 然后程序的内存分配/开辟都在这个内存池中进行
//由程序自己进行调度和优化 这样可以有效避免以上两个问题...可以了解一下有空的话

//到此 动态内存开辟函数的介绍就差不多了 学到这里 我们还可以对上节课的通讯录进行优化 将其更新为动态版本...
...
//2_对动态开辟空间的越界访问
//3_对非动态开辟内存使用free来释放
//4_使用free释放开辟内存的一部分 (free释放的空间必须是完整的先前开辟的空间而不能是一部分 free传递的指针必须是开辟空间的起始地址)
//5_对同一块动态内存多次释放 (其实就是操作到了已经不属于程序员的内存空间了)
//6_动态开辟内存但是忘记释放(内存泄漏):这个其实是最容易出错的 并且危害也是最大的 所以记住！动态开辟的内存空间一定要正确释放掉！


//常见错误示范：指向堆区空间的栈区指针被销毁的问题
//void Get_memory(char* p)
//{
//	p = (char*)malloc(100);//申请空间并将首地址放在p中
//	//但是 由于这里的p是局部变量 并且在退出函数前没有返回值 所以出p直接就被销毁了 并没有把这片空间的地址带出来
//	//这就导致内存空间开辟了 但是没有人能再使用它了 这也就是内存泄露了
//}
//void test(void)
//{
//	char* ptr = NULL;
//	Get_memory(ptr);
//	strcpy(ptr, "niubi");//由于没有返回 ptr仍然是NULL 意味着到strcpy函数中去时 程序会直接崩溃掉
//	printf(ptr);
//}
//int main()
//{
...
//}
//解决方式1：
//void Get_memory(char** p)//使用二级指针 先接受ptr的地址
//{
//	*p = (char*)malloc(100);//*p找到ptr malloc申请空间并将首地址放在ptr中
//}
//void test(void)
//{
//	char* ptr = NULL;
//	Get_memory(&ptr);//传ptr地址 以实现传址调用
//	strcpy(ptr, "niubi");//此时ptr中的地址已经更改
//	printf(ptr);
//	free(ptr);//勿忘回收内存空间
//	ptr = NULL;//勿忘赋值为空指针 避免ptr成为野指针
//}
//int main()
//{
//	test();
...
//}
//解决方式2：
//char* Get_memory(char* p)//使这个函数返回p的地址
//{
//	p = (char*)malloc(100);
//	return p;
//}
...
//{
//	char* ptr = NULL;
//	ptr=Get_memory(ptr);//将返回的地址再放入ptr中
//	strcpy(ptr, "niubi");//此时ptr中的地址已经更改
//	printf(ptr);
//	free(ptr);//勿忘回收内存空间
//	ptr = NULL;//勿忘赋值为空指针 避免ptr成为野指针
...
//错误示例2：返回被销毁栈空间中的地址问题
//int* test()
//{
//	int a = 10;//a是函数中的局部变量 在函数退出时a变量空间会被销毁 这片空间回归系统调度
//	return &a;//函数退出时变量的地址仍然存在寄存器中(仍是归系统调度) 所以这里可以正常返回
//}
//int main()
//{
//	int* p = test();//将函数中a的地址放在p中
//	//printf("hehe\n"); 加上这句语句后打印失败 因为这里使用了函数先前使用的空间 导致原先的a被修改成随机值
//	printf("%d\n", *p);//打印p指向的对象的值
//	//首先这种做法本身就是错误的 因为函数在栈区开辟的空间一旦回归系统调度 其中一切都不再交给程序员使用 此时的p也就是一个野指针
//	//但是这里的做法并不一定就不能实现目的 例如这里 我们发现竟然可以照常打印出a的值
//	//这是因为 这片空间回归系统后可能还没有被立刻调用 所以其中的内容还没有被格式化 这时我们调用这片空间就能访问到原先的值
//	//但是这种做法绝对是不允许的！
//	return 0;
//}
...
//内存6区：
//1_内核区	由系统调度 程序员无权使用/读写
//2_栈区	向下增长 由系统管理 在运行期间分配
//内存映射段 文件映射 动态库 匿名映射
//3_堆区	向上增长 由程序员管理 运行时分配 用完需要释放
//4_静态区	又称数据段 存放全局/静态变量的区域
//5_代码段	可执行代码/只读常量
//需要注意的是 这些区之间并不都是紧挨着的 例如栈区向下还有空隙 堆区向上同样也有空隙

//计算最大公约数
//法1_取两数较小数 不断递减 直至同时%上两者后都等于0
//法2_利用辗转相除法 a%b=r a=b,b=r--->>>直到r=0时 返回此时的除数即是最大公约数

//计算最小公倍数
//法1_取两数较大数 不断递增 直至同时%上两者后都等于0
//法2_利用辗转相除法 计算出最大公约数 然后用两数相乘再除以最大公约数就可以得到最大公约数


//柔性数组： 在C99中 结构体的最后一个元素允许是大小未知的数组 这就称为柔性数组成员
//struct stu
//{
//	int a;
//	int arr[];//这里的arr就是柔性数组成员 这里的[]也可以替换成arr[0] 只是有的编译器上会报错
//};
//柔性数组的特点：
//1_柔性数组成员前必须要有至少一个别的成员
//2_sizeof计算有柔性数组的结构体大小时 不会计算柔性数组的大小
//3_包含柔性数组的结构应用malloc等动态开辟函数进行内存的动态分配 并且分配的大小不小于结构的大小
...
//int main()
//{
//	s* ps = (s*)malloc(sizeof(s) + 40);//开辟空间 40是给arr分配的 也可以直接写44 这里只是提高可读性
//	if (ps==NULL)
//	{
//		return 1;
//	}
//	s* ptr = (s*)realloc(ps, sizeof(s) + 80);//使用realloc实时调整数组大小
//	if (ptr==NULL)
//	{
//		return 1;
//	}
...
//	return 0;
//}
//可见 柔性数组的柔性体现在它是在堆区空间上开辟的 所以可以由程序员来分配 提高了数组大小的灵活性

//以指针实现类似柔性数组的功能：
//typedef struct s
//{
//	int a;
//	int* arr;//用一个指针arr来接收后续动态开辟时分配的空间
//}s;
//int main()
//{
//	s* ps = (s*)malloc(sizeof(ps->a));//这里为了与柔性数组(成员整体均在堆区上申请空间)做对比 将s中的成员a也在堆区上申请空间
//	if (ps==NULL)
//	{
//		return 1;
//	}
//	ps->arr = (int*)malloc(40);//对ps中的arr也申请空间
//	int* p = (int*)realloc(ps->arr, 80);//使用realloc调整空间大小
//	if (p==NULL)
//	{
//		return 1;
//	}
...
//	return 0;
//}
//事实上 你会发现 用代码1和2均可以实现相似的功能 但是我们还是建议使用方式1 这也是柔性数组存在的意义
//第一个好处是: 使用柔性数组更便于内存释放 因为只需要对这片空间整体进行释放即可
//但是如果是后者 那么除了要释放前面成员的空间 还要释放最后一个指针的空间 这两步不能一起释放 就容易导致遗忘内存释放
//第二个好处是：连续的开辟内存空间有利于减少内存碎片以及提高访问速度


//宏的介绍
//什么是宏？ 在C语言中 我们允许使用#define来定义一个宏 该命令允许把一个名称指定成任何所需要的文本(例如一个常量值/语句)
//注意 源文件中任意地方都可以定义宏 宏的作用范围是定义处到末尾 宏的替换过程发生在预处理阶段
//使用#undef可以提前结束宏的作用范围

//使用建议：
//1_如果定义宏时有参数 那么建议在定义时将参数用括号括起来 避免传参后产生优先级的冲突
//2_如果宏定义需要多条定义语句 建议使用 do{...}while(0)进行封装 因为可能会出现宏替换处只能放置一条语句 例如if后面(没有{})的情况

//#和##
//两者都是C语言中的预处理指令 均只能在宏定义中使用
//1_当在宏定义中出现 #参数 的指令时 意为将参数的字面值转换为字符串
//#define st(s) #s//此处宏定义一个st函数 参数为s #s将参数的字面值转换为字符串
//int main()
//{
//	printf("%s\n", st(23*3));//输出"23*3"
//	printf("%d\n", st(23*3));//输出一个极大数 因为替换后这里的内容就变成了代码段中的一段二进制码(字符串) 以整形打印就得到了...
//	return 0;
//}
//2_如果出现了aa##bb##cc 这种使用双##定义的宏 这里的作用就是形成一个新的名字aabbcc ！注意 不是字符串 是名字-->变量或者常量的名字
//#define cmbn(a,b) a##b
//int main()
//{
//	int xy = 100;//这里需要事先对xy进行定义 否则后续cmbn合成名字xy后发现xy是未定义的 编译器会报错
//	int x = 1;
//	int y = 2;
//	printf("%d\n", cmbn(x, y) * y);//此时的cmbn将x,y合成了为一个xy(名字 这里是一个变量名 因为已定义的xy就是一个int变量)
//	//那么这里使用cmbn(x,y)就等同于使用xy*y 所以打印200
//	return 0;
//}
//这段代码还可以这么写：
//#define cmbn(a,b) a##b
//#define xy 100//宏定义一个常量xy 值是100
//int main()
//{
//	int x = 1;
//	int y = 2;
//	printf("%d\n", cmbn(x, y) * y);//照样打印200 因为此时xy被宏定义为一个值为100的常量 xy也就是一个常量名了这里
//	return 0;
//}

//去注释VS宏替换
//我们知道 预处理期间不仅要进行宏替换 还要进行去注释的操作 也就是说注释掉的内容不会进入到编译的过程
//那么 到底是先去注释呢 还是先进行宏替换呢？ 观察下面这段代码：
//#define ABS //		//这里将ABS宏定义为注释符
//这里 我们首先要知道 注释的原理就是将代码/文本改为字符'\0' 也就是空字符 然后编译器自然而然地的无视掉这些空字符
//int main()
//{
//	ABS printf("hehe\n");//这里 如果宏替换优先于去注释 那么就不会打印hehe
//	//反之如果这里打印了 就说明去注释优先于宏替换 因为去注释后 ABS后面就变成空字符了 相当于宏定义了ABS为空字符 下面也就会打印hehe
//	//------>>>>>最后的结果是打印了hehe这里 这说明去注释在宏替换之前！再次提醒 去注释在宏替换之前！！！
//	return 0;
//}
//事实上 这也比较好理解 因为去注释时还不必对代码主体进行改动 而宏替换却是要对代码主体进行改动 按步骤来说也应该是前者先
//此外 如果宏替换先 那么当注释中有宏替换的对象时 岂不是还要额外占用时间来替换掉这些注释段中的内容? 这有什么意义呢/

//条件编译
//什么是条件编译? 一般情况下 源文件中所有的非注释行都要参与编译 但是有时 我们希望其中部分内容只在满足条件时才进行编译
//这样一可以节省时间 二可以节省空间 而条件编译就可以实现这一点 在预处理阶段 如果条件编译的代码块满足条件则编译 不满足则不编译
//常见的条件编译指令有#ifdef #if #else #elif #ifdefined() #endif等		下面给出一些示例:
//#define money 0 假定此处没有宏定义money
//#define free 0//假定此处宏定义了free
//int main()
//{
//#ifdef money//意为 如果宏定义了money 则...
//	{
//		printf("money\n");//你会发现 还未编译 这行代码就已经变成浅色了 表明这段代码是不会被编译的
//	}
//#else //意为如果没有宏定义money则...
//	{
//		printf("free\n");//此处由于前面宏定义了free 所以这段代码时正常颜色的 也表明了它是要编译的
//	}
//#endif//意为终止条件编译代码段 任何条件编译都要以#endif收尾 这样编译器才能将其视为条件编译代码段的整体 也才能够执行条件编译
//	return 0;
//}//并且 #if #elif的判断功能都只局限于判断是否宏定义了... 而不会在意其真假(1/0) 此外#ifndef和#ifdef功能相反 前者的n是not缩写..
//如果还要实现判断是否为真/假 则还可以采用下面的方式：
//#define x 2//这里宏定义x为常量2
//int main()
//{
//#if x==0//意为如果x=0则.. 这里需要注意#if不同于#ifdef 需要判断表达式真假值 并且如果判断的内容未定义 则默认为假 也就是x会默认为0
//	{
//		printf("0");//----->>>那么这里就会打印字符"0"
//	}
//#elif x==1//#if不成立时如果x=1则..
//	{
//		printf("1");//同样由于x!=1 所以语句块变灰色
//	}
//#elif x==2//#if和前面的#elif不成立时如果x=2则..
//	{
//		printf("2");//打印2 
//	}
//#else//如果前面的条件都不满则则..  很容易联想到if中的多分支结构:)
//	{
//		printf("no findings\n");//由于前面有选择项命中了 所以这里也是灰色的 不会被编译
//	}
//#endif//还是作为条件编译的收尾 这也可以看出无论任何#if..条件编译指令开头的最终都需要#endif收尾！
//	return 0;
//}
//补充！#ifdef ..和#if defined(..)功能一致 #ifndef ..和#if !defined(..)功能也一致
//但是注意! 如果四者中出现一个 那么在下一个#if...前必须要先用#endif收尾！ 否则编译报错 因为#if...没有匹配项

//条件编译的作用：
//1_我们可以使用条件编译来裁剪代码 用于快速实现某种目的 例如版本维护,更新/功能裁剪/提高代码跨平台性等等
// 因为这样的需求在条件编译代码中只需要更改/维护一份代码即可 而如果要实现功能变更 也只需要改变特定的宏即可
//2_一个C工程中一般都有多个.c的源文件 而这些源文件中又很可能引用了多个相同头文件 这样会导致大量重复代码被拷贝 降低后期编译效率
// 因此我们可以通过条件编译来实现避免头文件的重复包含 如#ifndef _FUN_H_ 当后面有.c文件想包含fun.h时 这段代码会自动被裁剪掉
// 这也就实现了防止头文件被重复包含的功能 此外 还可以通过#pragma once预处理指令防止头文件重复包含！

//总结：
//1_预处理的步骤分别是：头文件展开-->去注释-->宏替换-->条件编译
//2_宏替换只是文本替换 使用时要考虑多种情况
//3_条件编译可以用于提升程序可维护性以及避免头文件重复包含等场景



//------------------------->>>>>>写在末尾
//初心易得 始终难守 第一次这么强烈地感觉到自己仿佛是完成了一项伟大事业般的成就感hh
//到此 C语言进阶的内容也基本结束了(除了文件的打开与读写的相关知识..后续可以自行了解)
//半年时间的笔记写到这里已经是7200行了 虽然没写到10000行 但是也已经完成了学C的心愿 这篇对C的笔记记录就到此为止吧
//后面可能就是学习C++和数据结构与算法了 祝好运吧 继续努力 加油:)