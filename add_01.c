...
//	 printf("%zu\n", sizeof(double));		8
//	 printf("%zu\n", sizeof(void));			0
//   //值得一提的是 C语言为了保持跨平台的统一 所以char; short int;int;long long int;float;double;void等都采用了固定位数
//	 //不随系统位数变化而变化 但是long int类型和指针变量类型的大小是根据系统位数而变化的 并且这两者是始终相等的
...
//在X86(32位)硬件环境中 可能的内存地址数(寻址范围)是2^32次方 计算如下：(地址为32bit位 二进制 故有2^32次方种可能)
//而寻址空间(寻址范围的最大值)则是 2^32/1024^3=4GB 注意 寻址范围(即地址总线宽)是没有单位的 但是寻址空间是有单位的
//2^32=4294967296Byte
//	=4194304Kb
//	=4096Mb
//	=4Gb 即4Gb个
//而在X64硬件系统中 寻址范围达到了惊人的2^64 而寻址空间更是达到了16EB！
//但是受主板 运行效率等等限制 目前是不可能达到这样大的寻址空间的
//换算关系:
//8 bit =1Byte
//1024 Byte=1Kb
//1024 Kb=1Mb
...
//	//提问：3^3=? 答案是0 因为这两数是完全相同的 那么0^3=? 答案是3；0^8=? 答案是8
//	//所以我们能得出一下结论：
//	//a^a=0 0^a=a
...
//以字符串之间的比较为例：
//如果我们采用"abc"=="abcdef" 那么我们实际上是在比较两个字符串的(首字符)地址是否相等 而不是比较两个字符串的内容是否相等
