...
//int main()
//{
//	int* p;//这并不是初始化 只能说定义了一个int*类型的指针 其中放的是随机地址 0xcccccccc
...
//	for (int i = 0; i <=10; i++)
//	{
//		*(p++) = i;//当第十一次进入循环时 p已经前进到超过了整个数组长度 那么p所指向的位置就不确定了 p可能+1/2/4/8..
...
//1_指针一定要初始化
//2_小心指针的越界
//3_指针指向空间被释放/当前不使用该指针/当前不知道要指向哪个空间--->放置空指针NULL(即0)
//int main()
//{
