...
//	signed char b = -1;//signed char占8个bit位 第一位为符号位不计入大小 所以表示范围是-2^7 ~ 2^7-1 即-128到127
//	unsigned char c = -1;//unsigned char 无符号位 表示范围是0到2^8-1 即0到255
//	//前面讲过 用无符号类型来看待负数时其实就是将负数的补码看作是正数的原码
//	printf("%d %d %d\n", a, b, c);//-1 -1 255
//	return 0;
//}
