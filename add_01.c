...
//不过VS编译器是不支持变长数组的，而别的一些编译器如小熊猫的GCC编译器是支持的
...
//冒泡排序算法核心：用相邻的两个元素进行比较！
//现有：9 8 7 6 5 4 3 2 1 0 请将其改为升序
//冒泡的思路: 相邻元素比较 9>8，故交换；9>7，故交换；...直到9交换到最后一位不能再交换时 这一次冒泡排序才结束
//由于冒泡本质就是相邻两数的交换 而排序过程神似饮料中气泡上浮的过程 因此得名冒泡排序
//注意区分：冒泡和冒泡排序！ 一次冒泡是指相邻两数交换一次的过程 而一次冒泡排序指一个数据从开始冒泡到结束冒泡的一整个过程
//不难理解 10个数时第一次冒泡排序只需要冒泡9次(插空为9)，而总体排序需要的也是9次冒泡排序 
//但当第二次冒泡排序时 总计需要八次冒泡排序 而这一次冒泡排序需要冒泡8次 也就是说剩余冒泡排序的次数和冒泡的次数存在着1对1的对应关系
int main()
{
